...
$.jqPaginator('#pagination', {
    ...
    onPageChange: function(num, type) {
        $.ajax({
            type: "get",
            cache: true,
            url:"/api/article/tags?tag="+tag+"&page="+num,
            dataType: 'json',
            async: true,
            success: function(result) {
                totalPageasd = result.data.last_page
                var str = '';
                    ...
                    <div class="entry-text">
                        <div class="entry-header">
                            ...
                            <h1 class="entry-title"><a href="single.html?article=${result.data.data[i].id}">${result.data.data[i].title}</a></h1>
